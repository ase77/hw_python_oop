...
def test_read_package():
    ...
    assert isinstance(
        homework.read_package, types.FunctionType
    ), "`read_package` должна быть функцией."
...
def test_Training_show_training_info(monkeypatch):
    ...
    monkeypatch.setattr(
        training, "get_spent_calories", mock_get_spent_calories
    )
...
def test_Swimming():
    ...
    assert issubclass(
        homework.Swimming, homework.Training
...
def test_SportsWalking():
    ...
    assert issubclass(
        homework.SportsWalking, homework.Training
...
def test_SportsWalking_get_spent_calories(input_data, expected):
    ...
    assert hasattr(
        sports_walking, "get_spent_calories"
...
def test_Running():
    ...
    assert issubclass(
        homework.Running, homework.Training
...
def test_main():
    ...
    assert isinstance(
        homework.main, types.FunctionType
